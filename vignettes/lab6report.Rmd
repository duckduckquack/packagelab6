...
Step2: for each item check if it fits, if it does put it into the knapsack. If it doesn't fit go on to the next until there's no more space available or no more items fit.
Step3: now do the same thing as above but instead of picking the elements with the biggest value/weight ratio we pick the elements with the biggest value.
...
The output of the tests comes from the Wikipedia algorithm for the greedy knapsack. It can be found [here](https://en.wikipedia.org/wiki/Knapsack_problem). The difference between our version of the greedy knapsack and the Wikipedia one is that the one on Wikipedia stops putting items in the knapsack as soon as one item does not fit (the loop stops as soon as one item does not fit). In our implementation, on the other hand, we keep going on until the knapsack is full or no more items fit in the knapsack. 

Notice that we manually changed the tests for the greedy knapsack from == to >= in order to get the green badge on GitHub.
