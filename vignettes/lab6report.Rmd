...
The microbenchmark function runs the algorithm multiple time to get a more accurate running time. As the unoptimized_brute_force_knapsack is really slow, we are going to run the microbenchmark 5 times.
```{r, eval = FALSE}
microbenchmark::microbenchmark(unoptimized_brute_force_knapsack(knapsack_objects[1:16, ], 2000), times = 5)
```
| min | lq  | mean | median | uq   | max |
|----:|----:|-----:|-------:|----: |----:|
|4.11 |4.22 | 5.19 | 5.07   | 4.25 | 4.35|
Unit: seconds

As you can see, we get a confirmation that our brute force implementation takes between 4 and 6 seconds (we look at the median) to run with an input size of only 16 objects. 
...
The microbenchmark function runs the algorithm multiple time to get a more accurate running time. We are going to run the microbenchmark 5 times.
```{r, eval = FALSE}
microbenchmark::microbenchmark(knapsack_dynamic(knapsack_objects[1:500, ], 2000), times = 5)
```
| min   | lq    | mean    | median | uq     | max    |
|------:|------:|---------|-------:|-------:|-------:|
|698.64 |835.28 | 1466.43 | 922.27 | 975.48 | 3900.46|
Unit: milliseconds

As you can see above, we get a confirmation that our algorithm takes around 900 milliseconds to run on 500 objects. Which considering the bigger input size, it's a huge performance improvement compared to the brute force version. 
...
```
As you can see, from working with a data frame to working with a vector with obtained a good performance improvement. 
...
```{r, eval = FALSE}
microbenchmark::microbenchmark(brute_force_knapsack(knapsack_objects[1:16, ], 2000), times = 5)
```
| min   | lq    | mean    | median | uq     | max    |
|------:|------:|---------|-------:|-------:|-------:|
...
As you can see this is a huge improvements from the unoptimized_brute_force_knapsack. However, compared to brute_force_knapsack, this parallelized version performs slightly worse. 
...
As you can see, we obtain an improvement from the parallelized version that also works on Windows machines. However, also this version is not better than the optimized brute force version on average. They perform equally as good. 
```{r, eval = FALSE}
...
Our implementation of the greedy heuristic knapsack algorithm returns a better result than the ones in the test for larger n (we don't have this problem for smaller input sizes). 
This is a description of the algorithm we used:

Step1: sort items in terms of the ratio value/weight in decreasing order
Step2: select the elements with the biggest value/weight ratio until they fit in the knapsack
...
Probably, this is due to the fact that we can have objects with the same ratio v/w due to rounding. In this case, certain implementation can take an item as opposed to another with the same ratio only because that item shows up first. This can lead to performance differences. 
